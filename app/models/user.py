...
class User:
    ...
    def __init__(self, telegram_id, service_type, score=0, invite_code=None, id=None, last_sign_in_date=None, username=None):
        self.id = id
    ...
    def save(self):
        ...
        if self.id:
            # 更新
            cursor.execute(
                "UPDATE Users SET telegram_id = ?, service_type = ?, score = ?, invite_code = ?, last_sign_in_date = ?, username = ? WHERE id = ?",
                (self.telegram_id, self.service_type, self.score, self.invite_code, self.last_sign_in_date, self.username, self.id)
            )
            logger.debug(f"更新用户数据: id={self.id}, telegram_id={self.telegram_id}, service_type={self.service_type}")
        else:
            # 插入
            cursor.execute(
                "INSERT INTO Users (telegram_id, service_type, score, invite_code, last_sign_in_date, username) VALUES (?, ?, ?, ?, ?, ?)",
                (self.telegram_id, self.service_type, self.score, self.invite_code, self.last_sign_in_date, self.username)
            )
    ...
    def get_by_telegram_id_and_service_type(telegram_id, service_type=None):
        ...
        if row:
            ...
            return User(row['telegram_id'], row['service_type'], row['score'], row['invite_code'], row['id'], row['last_sign_in_date'], row['username'])
        else:
    ...
    def get_all():
        ...
        return [User(row['telegram_id'], row['service_type'], row['score'], row['invite_code'], row['id'], row['last_sign_in_date'], row['username']) for row in rows]
    ...
    def __str__(self):
        return f"<User id={self.id}, telegram_id={self.telegram_id}, service_type={self.service_type}, score={self.score}, invite_code={self.invite_code}, last_sign_in_date={self.last_sign_in_date}, username={self.username}>"
...
class ServiceUser(User):
    ...
    def __init__(self, telegram_id, score=0, invite_code=None, id=None, service_user_id=None, last_sign_in_date=None, service_type='navidrome', username=None):
        super().__init__(telegram_id, service_type, score, invite_code, id, last_sign_in_date, username)
    ...
    def save(self):
        ...
        if self.id:
            # 更新
            cursor.execute(
                "UPDATE Users SET telegram_id = ?, service_type = ?, score = ?, invite_code = ?, service_user_id = ?, last_sign_in_date = ?, username=? WHERE id = ?",
                (self.telegram_id, self.service_type, self.score, self.invite_code, self.service_user_id, self.last_sign_in_date, self.username, self.id)
            )
            ...
            # 插入
            cursor.execute(
                "INSERT INTO Users (telegram_id, service_type, score, invite_code, service_user_id, last_sign_in_date, username) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (self.telegram_id, self.service_type, self.score, self.invite_code, self.service_user_id, self.last_sign_in_date, self.username)
            )
            self.id = cursor.lastrowid
    ...
    def get_by_telegram_id_and_service_type(telegram_id, service_type=None):
        ...
        if row:
           ...
           return ServiceUser(row['telegram_id'], row['score'], row['invite_code'], row['id'], row['service_user_id'], row['last_sign_in_date'], service_type=row['service_type'], username=row['username'])
        else:
    ...
    def get_all(service_type=None):
        ...
        return [ServiceUser(row['telegram_id'], row['score'], row['invite_code'], row['id'], row['service_user_id'], row['last_sign_in_date'], service_type=row['service_type'], username=row['username']) for row in rows]
    ...
    def __str__(self):
        return f"<ServiceUser id={self.id}, telegram_id={self.telegram_id}, service_user_id={self.service_user_id}, score={self.score}, invite_code={self.invite_code}, last_sign_in_date={self.last_sign_in_date}, username={self.username}>"
